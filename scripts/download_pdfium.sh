...
# 尝试多个下载源
DOWNLOAD_URLS=(
    "https://github.com/bblanchon/pdfium-binaries/releases/download/chromium%2F${PDFIUM_VERSION}/${FILENAME}"
    "https://github.com/bblanchon/pdfium-binaries/releases/download/chromium/${PDFIUM_VERSION}/${FILENAME}"
)
...
if [ "$DOWNLOADED" = false ]; then
    ...
    echo "1. Go to https://github.com/bblanchon/pdfium-binaries/releases"
...
if [ -f "$LIB_FILE" ]; then
    ...
    if [ "$PLATFORM" = "linux" ]; then
        ...
        echo ""
        echo "Or run with:"
        echo "  LD_LIBRARY_PATH=${TARGET_DIR}/lib ./your-program"
    elif [ "$PLATFORM" = "mac" ]; then
